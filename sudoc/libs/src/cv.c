...
{
    cv_image_rgb *image = malloc(sizeof(cv_image_rgb));

    image->width = width;
    image->height = height;
    image->data = malloc(sizeof(unsigned char **) * height);

    for (int i = 0; i < height; i++)
    {
        image->data[i] = malloc(sizeof(unsigned char *) * width);
        for (int j = 0; j < width; j++)
        {
            image->data[i][j] = malloc(sizeof(unsigned char) * 3);
        }
    }
...
/// @brief Copy an image from an array to a cv_image_rgb structure.
/// @param image Array containing the image.
/// @param width Width of the image.
/// @param height Height of the image.
/// @return Returns a pointer to the cv_image_rgb structure.
cv_image_rgb *cv_copy_image_rgb(unsigned char ***image, int width, int height)
{
    cv_image_rgb *copy = cv_init_image_rgb(width, height);
...
{
    cv_image_gray *image = malloc(sizeof(cv_image_gray));
...
/// @brief Copy an image from an array to a cv_image_gray structure.
/// @param image Array containing the image.
/// @param width Width of the image.
/// @param height Height of the image.
/// @return Returns a pointer to the cv_image_gray structure.
cv_image_gray *cv_copy_image_gray(unsigned char **image, int width, int height)
{
    cv_image_gray *copy = cv_init_image_gray(width, height);

    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            copy->data[i][j] = image[i][j];
        }
    }
...
{
    cv_image_rgb_normalized *image = malloc(sizeof(cv_image_rgb_normalized));
...
/// @brief Copy an image from an array to a cv_image_rgb_normalized structure.
/// @param image Array containing the image.
/// @param width Width of the image.
/// @param height Height of the image.
/// @return Returns a pointer to the cv_image_rgb_normalized structure.
cv_image_rgb_normalized *cv_copy_image_rgb_normalized(float ***image, int width, int height)
{
    cv_image_rgb_normalized *copy = cv_init_image_rgb_normalized(width, height);
...
{
    cv_image_gray_normalized *image = malloc(sizeof(cv_image_gray_normalized));
...
/// @brief Copy an image from an array to a cv_image_gray_normalized structure.
/// @param image Array containing the image.
/// @param width Width of the image.
/// @param height Height of the image.
/// @return Returns a pointer to the cv_image_gray_normalized structure.
cv_image_gray_normalized *cv_copy_image_gray_normalized(float **image, int width, int height)
{
    cv_image_gray_normalized *copy = cv_init_image_gray_normalized(width, height);
...
/// @brief Apply grayscale to a cv_image_rgb structure.
/// @param rgb Pointer to the cv_image_rgb structure.
void cv_grayscale(cv_image_rgb *rgb)
{
    cv_image_gray *gray = cv_init_image_gray(rgb->width, rgb->height);
    cv_rgb_to_gray(rgb, gray);
    cv_gray_to_rgb(gray, rgb);
    cv_free_image_gray(gray);
}

/// @brief Apply a Gaussian blur to a cv_image_rgb structure.
/// @param src Pointer to the cv_image_gray structure.
/// @param dst Pointer to the cv_image_gray structure.
